...
import { LayoutDashboard, Package, ShoppingCart, Receipt, Menu, X, Wine, LogOut, PartyPopper, ChefHat, Users, UtensilsCrossed, Coffee, Bell, ClipboardList, History, Banknote, DollarSign, ShoppingBasket, Bot, Ticket, ScanLine, ListChecks, FileImage } from 'lucide-react';
  ...
  return <>
      ...
      <aside className={cn("fixed left-0 top-0 h-full w-64 bg-sidebar border-r border-sidebar-border z-40 transition-transform duration-300 lg:translate-x-0", isOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex flex-col h-full">
          ...
          {/* User info */}
          {currentStaff && <div className="px-4 py-3 border-b border-sidebar-border bg-muted/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-sm truncate">{currentStaff.full_name}</p>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {roleLabels.map((label, idx) => <Badge key={idx} variant="secondary" className="text-xs">
                        {label}
                      </Badge>)}
                  </div>
                </div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="ghost" size="icon" className="relative">
                      ...
                      {unreadCount > 0 && <span className="absolute -top-1 -right-1 bg-destructive text-destructive-foreground text-xs w-5 h-5 rounded-full flex items-center justify-center">
                  ...
                  <PopoverContent className="w-80 p-0" align="end">
                    ...
                    <ScrollArea className="h-64">
                      {notifications.length === 0 ? <div className="p-4 text-center text-muted-foreground text-sm">
                          ...
                          {notifications.map(notification => <div key={notification.id} className="p-3 hover:bg-muted/50 cursor-pointer" onClick={() => markAsRead(notification.id)}>
          ...
          <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
            ...
            return <Link key={item.path} to={item.path} onClick={() => setIsOpen(false)} className={cn("nav-link", isActive && "nav-link-active")}>
